#!/usr/bin/env python
"""Run the SGCRL critic-architecture ablation from the paper.

This ad hoc experiment mirrors the representation ablation described in
Section 4.3 / Figure 11 of "A Single Goal is All You Need": compare the
standard CRL critic,

    q(s, a, g) = phi(s, a)^T psi(g),

against a monolithic critic,

    q(s, a, g) = f([s, a, g]).

The paper notes that this ablation used batch size 32 for both variants because
the monolithic all-pairs critic had higher memory use.
"""
...
from sgcrl_torch.networks import ContrastiveQNetwork, MLP, NetworkBundle, PolicyNetwork
from sgcrl_torch.runner import run_training


PAPER_REFERENCE = (
    "Liu, Tang, and Eysenbach, 'A Single Goal is All You Need', "
...
def _seeds(values: Iterable[str]) -> Sequence[int]:
    seeds = []
    for value in values:
        seeds.extend(int(part) for part in value.split(",") if part)
    return seeds


class MonolithicQNetwork(nn.Module):
    """Critic q(s, a, g) that scores every state-action/goal pair in a batch."""
    ...
    def __init__(
        ...
        super().__init__()
        if use_image_obs:
            raise NotImplementedError("MonolithicQNetwork only supports vector observations.")

        self.obs_dim = obs_dim
        self.goal_dim = goal_dim
        self.action_dim = action_dim
        self.twin_q = twin_q
        input_dim = obs_dim + action_dim + goal_dim
        self.q = MLP(input_dim, hidden_layer_sizes, 1)
        self.q2 = MLP(input_dim, hidden_layer_sizes, 1) if twin_q else None

    def _pair_inputs(self, obs: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
        state = obs[:, : self.obs_dim]
        goal = obs[:, self.obs_dim :]
        batch_size = state.shape[0]

        state_action = torch.cat([state, action], dim=-1)
        state_action_pairs = state_action[:, None, :].expand(batch_size, batch_size, -1)
        goal_pairs = goal[None, :, :].expand(batch_size, batch_size, -1)
        pair_inputs = torch.cat([state_action_pairs, goal_pairs], dim=-1)
        return pair_inputs.reshape(batch_size * batch_size, -1)

    def _score_pairs(self, network: nn.Module, obs: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
        batch_size = obs.shape[0]
        logits = network(self._pair_inputs(obs, action))
        return logits.reshape(batch_size, batch_size)

    def forward(self, obs: torch.Tensor, action: torch.Tensor):
        logits = self._score_pairs(self.q, obs, action)
        if self.twin_q:
            logits2 = self._score_pairs(self.q2, obs, action)
            logits = torch.stack([logits, logits2], dim=-1)

        # ContrastiveLearner ignores the representation outputs, but returns
        # them for API compatibility with ContrastiveQNetwork.
        empty_repr = obs.new_empty((obs.shape[0], 0))
        return logits, empty_repr, empty_repr


def make_networks_for_arch(config: ContrastiveConfig, critic_arch: str) -> NetworkBundle:
    ...
    if critic_arch == "product":
    ...
    elif critic_arch == "monolithic":
        critic = MonolithicQNetwork(
            obs_dim=config.obs_dim,
            goal_dim=config.resolved_goal_dim(),
            action_dim=config.action_dim,
            hidden_layer_sizes=config.hidden_layer_sizes,
        ...
        raise ValueError(f"Unknown critic architecture: {critic_arch}")

    return NetworkBundle(policy=policy, critic=critic)
...
def resolve_checkpoint_template(template: str | None, args, seed: int, critic_arch: str) -> str | None:
    if not template:
        return None
    return template.format(seed=seed, variant=critic_arch, env=args.env, alg=args.alg)


@contextlib.contextmanager
def patched_network_factory(critic_arch: str):
    original_make_networks = learner_module.make_networks

    def _make_networks(config):
        return make_networks_for_arch(config, critic_arch)

    learner_module.make_networks = _make_networks
    try:
        yield
    finally:
        learner_module.make_networks = original_make_networks
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run the SGCRL product-vs-monolithic critic ablation."
    )
    parser.add_argument("--log_dir_path", default="logs/critic_arch_ablation/", help="Where to log metrics")
    parser.add_argument("--time_delta_minutes", type=int, default=5, help="Checkpoint cadence metadata")
    parser.add_argument("--seeds", nargs="+", default=["0"], help="Seeds, e.g. `0 1 2 3 4` or `0,1,2,3,4`")
    parser.add_argument("--add_uid", action="store_true", help="Save logs in a unique subdirectory")
    parser.add_argument(
        "--variants",
        nargs="+",
        default=["product", "monolithic"],
        choices=["product", "monolithic"],
        help="Critic architectures to run",
    )
    parser.add_argument(
        "--alg",
        default="contrastive_cpc",
        choices=["contrastive_nce", "contrastive_cpc", "c_learning", "nce+c_learning"],
    )
    parser.add_argument("--env", default="point_Spiral11x11", help="sawyer_bin, sawyer_box, sawyer_peg, point_Spiral11x11")
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Load policy, critic, target critic, optimizers, and alpha state from a learner checkpoint. "
            "Supports {seed}, {variant}, {env}, and {alg} placeholders."
        ),
    )
    ...
    parser.add_argument("--batch_size", type=int, default=32, help="Paper ablation used 32 for both variants")
...
def build_config(args, seed: int, critic_arch: str) -> ContrastiveConfig:
    ...
    config.alg_name = f"{args.alg}_{critic_arch}"
    config.critic_architecture = critic_arch
    return config
...
def main():
    ...
    print("Experiment: product critic phi(s,a)^T psi(g) vs monolithic critic f([s,a,g]).")
    print(f"Using batch size {args.batch_size}; the paper used 32 for this ablation.")

    for seed in seeds:
        for critic_arch in args.variants:
            config = build_config(args, seed, critic_arch)
            checkpoint_path = resolve_checkpoint_template(
            ...
            print(
                f"Starting {critic_arch} critic run: env={args.env}, alg={args.alg}, "
                f"seed={seed}, log_dir={args.log_dir_path}"
            )
            with patched_network_factory(critic_arch), patched_warm_start(checkpoint_path):
                run_training(config)
